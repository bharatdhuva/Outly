...
export function scheduleFollowUpChecker() {
  if (env.FOLLOWUP_ENABLED === "false") {
    logger.info("Follow-up engine is disabled in config.", { source: "jobos" });
    return;
  }

  // Runs on the defined FOLLOWUP_CHECK_CRON schedule, e.g., 0 */6 * * * (Every 6 hours)
  cron.schedule(env.FOLLOWUP_CHECK_CRON || "0 */6 * * *", async () => {
    logger.info("Running FollowUp Checker Cron Job...", { source: "jobos" });
    ...
    if (dueCompanies.length === 0) {
      logger.info("No companies due for follow-up right now.", { source: "jobos" });
      return;
    }

    logger.info(`Found ${dueCompanies.length} companies due for follow-up. Generating emails...`, { source: "jobos" });
    ...
    for (const company of dueCompanies) {
      ...
      try {
        ...
        if (followUp && followUp.body && followUp.subject) {
          ...
          logger.info(`Queued follow-up for ${company.company_name}`, { source: "jobos" });
        ...
        logger.error(`Failed to generate follow up for ${company.company_name}`, { error: String(err), source: "jobos" });
      }
    }
  });

  logger.info(`FollowUp Checker scheduled: ${env.FOLLOWUP_CHECK_CRON || "0 */6 * * *"}`, { source: "jobos" });
}
