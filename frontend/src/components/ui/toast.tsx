...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Viewport
    ref={ref}
    className={cn(
      "fixed top-4 left-1/2 -translate-x-1/2 z-[100] flex max-h-screen w-[90%] max-w-[360px] flex-col p-4 md:top-16 md:right-4 md:left-auto md:translate-x-0 md:max-w-[380px] font-sans",
      className,
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Action
    ref={ref}
    className={cn(
      "inline-flex h-8 shrink-0 items-center justify-center rounded-xl border bg-transparent px-3 text-sm font-medium ring-offset-background transition-colors group-[.destructive]:border-neutral-800 hover:bg-secondary group-[.destructive]:hover:bg-neutral-800 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none disabled:opacity-50",
      className,
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Close
    ref={ref}
    className={cn(
      "absolute right-3 top-3 rounded-lg p-1 text-current opacity-40 hover:opacity-100 transition-opacity focus:opacity-100 focus:outline-none",
      className,
    )}
    toast-close=""
    {...props}
