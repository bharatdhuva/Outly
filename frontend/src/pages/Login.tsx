...
export default function Login() {
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      ...
      toast({
        title: "Waking up the cloud server... 🌌",
        description: "The free hosting tier goes to sleep after inactivity. We are booting it up for you. This will take about 15-30 seconds, please hold on!",
        duration: 10000,
      });
    }, 2000);
      ...
      .then(() => {
        ...
        if (isSlow) {
          toast({
            ...
            description: "The server is awake and ready. You can now log in instantly.",
          });
        }
      })
      .catch(() => {
  ...
  return (
    <div className="flex flex-col lg:flex-row min-h-screen w-full bg-[#F2EBDD] font-sans">
      ...
      <div className="hidden lg:flex w-1/2 bg-[#F2EBDD] p-12 lg:p-16 flex-col justify-between relative overflow-hidden select-none sticky top-0 h-screen shrink-0">
        ...
        {/* Logo Header */}
        <div className="flex items-center z-10 cursor-pointer hover:opacity-80 transition-opacity self-start" onClick={() => navigateTo("/")}>
          <img src={logoTransparent} alt="Outly Logo" className="h-8 w-auto object-contain" />
        </div>
      ...
      <div className="w-full lg:w-1/2 p-6 lg:p-16 flex flex-col justify-center items-center min-h-screen bg-[#FAF6F0]">
        <div ref={formWrapperRef} className="w-full max-w-[420px] flex flex-col justify-center will-change-transform py-12 lg:py-16 lg:min-h-[620px]">
          ...
          {/* Mobile Logo */}
          <div className="flex items-center lg:hidden mb-8 self-start cursor-pointer" onClick={() => navigate("/")}>
            <img src={logoTransparent} alt="Outly Logo" className="h-6 w-auto object-contain" />
          </div>
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            <button
              type="submit"
              className={`w-full bg-outly-accent text-white py-3.5 rounded-full font-sans text-[15px] font-medium transition-all duration-200 shadow-sm hover:opacity-95 active:scale-[0.98] select-none flex items-center justify-center gap-2.5 cursor-pointer ${isLoading || isWakingServer ? "opacity-85 pointer-events-none" : ""
                }`}
            >
              {isLoading ? (
                <>
